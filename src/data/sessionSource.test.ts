 ...
 * real response.
 *
 * Trimmed to the timezone contract: where the tenant's zone comes from, and
 * what happens when the connection carries none.
 */
...
function harness(opts: { routes?: Record<string, unknown>; conns?: unknown[] } = {}) {
  const routes: Record<string, unknown> = opts.routes ?? {};
    ...
    if (path === "/api/v1/bootstrap") payload = { data: { csrfToken: "csrf-abc" } };
      ...
      payload = {
        connections: opts.conns ?? [
          { id: "conn-1", name: "Kitchen DB", timezone: "Europe/Lisbon", currency: "EUR" },
        ],
      };
...
/* `orders` is mapped but ABSENT from SCHEMA_OK on purpose: the two failures
...
const SCHEMA_OK = {
  "/api/v1/connections/conn-1/schema": {
    model: {
      tables: [
        { name: "menu_categories", columns: [{ name: "id" }, { name: "name" }] },
        { name: "menu_items", columns: [{ name: "id" }, { name: "category_id" }] },
      ],
    },
  },
};
...
describe("discovery", () => {
  ...
  it("says the connection is PAUSED rather than missing when it is", async () => {
     ...
     */
    const { fetchImpl } = harness({ conns: [{ id: "a", name: "Kitchen DB", disabled: true }] });
    const port = sessionPort({ tableOfRef: TABLE_OF_REF, fetchImpl });
    await expect(port.config()).rejects.toMatchObject({ code: "CONNECTION_PAUSED" });
    await expect(port.config()).rejects.toThrow(/Kitchen DB/);
  });
  ...
  it("points at a PAUSED connection when the schema does not match", async () => {
    ...
    const { fetchImpl } = harness({
      ...
      conns: [
        ...
        { id: "conn-2", name: "c_online_ordering", disabled: true },
      ],
    });
    const port = sessionPort({ tableOfRef: TABLE_OF_REF, fetchImpl });
    ...
    expect(message).toContain("c_online_ordering is paused in Adminium");
