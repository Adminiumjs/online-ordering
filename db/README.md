...
`docker compose up` gives you a working product: `ordering-db` creates the
...
By default these talk to the `ordering-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/juniper npm run demo:status
```
